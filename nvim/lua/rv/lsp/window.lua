...
function M.fancy_floating_markdown(contents, opts)
  ...
  local content_opts = {
    contents = stripped,
    filetype = "sagahover",
    highlight = "LspHoverBorder",
  }

  -- Make the floating window.
  local bufnr, winid = M.create_win_with_border(content_opts, opts)
  local height = opts.height or #stripped
  api.nvim_win_set_var(0, "LSP_hoverwin_data", { winid, height, height, #stripped })

  api.nvim_buf_add_highlight(bufnr, -1, "LspDocTruncateLine", wraped_index, 0, -1)
