local api = vim.api
local libs = {}
local server_filetype_map = require("rv.lsp").config_values.server_filetype_map
...
function libs.check_lsp_active()
  ...
  if next(active_clients) == nil then
    return false, "[LSP] No lsp client available"
  end
  return true, nil
end
...
function libs.result_isempty(res)
  if type(res) ~= "table" then
    print "[LSP] Server return wrong response"
    return
  end
...
function libs.apply_keys(ns)
  return function(func, keys)
    ...
    local fmt = "nnoremap <buffer><nowait><silent>%s <cmd>lua require('rv.lsp.%s').%s()<CR>"
