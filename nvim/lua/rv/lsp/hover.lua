...
function hover.has_saga_hover()
  local has_hover_win, datas = pcall(api.nvim_win_get_var, 0, "LSP_hoverwin_data")
...
function hover.close_hover_window()
  if hover.has_saga_hover() then
    local data = npcall(api.nvim_win_get_var, 0, "LSP_hoverwin_data")
...
function hover.scroll_in_hover(direction)
  local has_hover_win, hover_data = pcall(api.nvim_win_get_var, 0, "LSP_hoverwin_data")
  if not has_hover_win then
    return
  end
  ...
  api.nvim_win_set_var(0, "LSP_hoverwin_data", { hover_win, height, current_win_lnum, last_lnum })
end
