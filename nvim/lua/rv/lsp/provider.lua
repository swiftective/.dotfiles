local window = require "rv.lsp.window"
local vim, api, lsp, vfn = vim, vim.api, vim.lsp, vim.fn
local config = require("rv.lsp").config_values
local libs = require "rv.lsp.libs"
local home_dir = libs.get_home_dir()
local scroll_in_win = require("rv.lsp.action").scroll_in_win
...
function Finder:render_finder_result()
  ...
  api.nvim_command "highlight! link CursorLine LspFinderSelection"
  api.nvim_command 'autocmd CursorMoved <buffer> lua require("LSP.provider").set_cursor()'
  api.nvim_command 'autocmd CursorMoved <buffer> lua require("LSP.provider").auto_open_preview()'
  api.nvim_command "autocmd QuitPre <buffer> lua require('LSP.provider').close_lsp_finder_window()"
...
function Finder:apply_float_map()
  ...
  local nvim_create_keymap = require("rv.lsp.libs").nvim_create_keymap
  local lhs = {
    noremap = true,
    silent = true,
  }
  local keymaps = {
    { self.bufnr, "n", action.vsplit, ":lua require'rv.lsp.provider'.open_link(2)<CR>" },
    { self.bufnr, "n", action.split, ":lua require'rv.lsp.provider'.open_link(3)<CR>" },
    { self.bufnr, "n", action.scroll_down, ":lua require'rv.lsp.provider'.scroll_in_preview(1)<CR>" },
    { self.bufnr, "n", action.scroll_up, ":lua require'rv.lsp.provider'.scroll_in_preview(-1)<CR>" },
  }

  if type(action.open) == "table" then
    for _, key in ipairs(action.open) do
      table.insert(keymaps, { self.bufnr, "n", key, ":lua require'rv.lsp.provider'.open_link(1)<CR>" })
    end
  elseif type(action.open) == "string" then
    table.insert(keymaps, { self.bufnr, "n", action.open, ":lua require'rv.lsp.provider'.open_link(1)<CR>" })
  end

  if type(action.quit) == "table" then
    for _, key in ipairs(action.quit) do
      table.insert(keymaps, { self.bufnr, "n", key, ":lua require'LSP.provider'.close_lsp_finder_window()<CR>" })
    end
  elseif type(action.quit) == "string" then
    table.insert(keymaps, { self.bufnr, "n", action.quit, ":lua require'LSP.provider'.close_lsp_finder_window()<CR>" })
  end
...
function Finder:open_link(action_type)
  ...
  if self.short_link[current_line] == nil then
    error "[Lsp] target file uri not exist"
    return
  end
...
function lspfinder.preview_definition(timeout_ms)
  ...
  if vim.tbl_islist(result) and not vim.tbl_isempty(result[1]) then
    ...
    api.nvim_buf_set_var(0, "LSP_def_preview", { wi, 1, config.max_preview_lines, 10 })
  end
end

-- function lspfinder.has_saga_def_preview()
--   local has_preview, pdata = pcall(api.nvim_buf_get_var, 0, "LSP_def_preview")
...
--   api.nvim_buf_set_var(0, "LSP_def_preview", { pdata[1], current_win_lnum, config.max_preview_lines, pdata[4] })
-- end
