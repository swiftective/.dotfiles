...
local action = require "rv.lsp.action"
local implement = {}

function implement.LSP_implementation(timeout_ms)
  ...
  if vim.tbl_islist(result) and not vim.tbl_isempty(result[1]) then
    ...
    api.nvim_buf_set_var(0, "LSP_implement_preview", { wi, 1, config.max_preview_lines, 10 })
  end
end

function implement.has_implement_win()
  local has_win, data = pcall(api.nvim_buf_get_var, 0, "LSP_implement_preview")
  if not has_win then
    return false
  end
...
function implement.scroll_in_implement(direction)
  local has_hover_win, data = pcall(api.nvim_win_get_var, 0, "LSP_implement_preview")
  ...
  api.nvim_win_set_var(0, "LSP_implement_preview", { hover_win, height, current_win_lnum, last_lnum })
end
return implement
